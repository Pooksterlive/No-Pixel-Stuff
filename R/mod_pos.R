...
mod_pos_server <- function(id, State_ID, changed = reactiveVal(0)) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$checkout, {
      ...
      tx <- data.frame(transaction_id = tx_id, transaction_number = tx_number, State_ID = as.integer(State_ID()), total = total(), payment_method = input$payment, created_at = as.character(Sys.time()), stringsAsFactors = FALSE)
      write_csv(rbind(transactions, tx), file_paths$transactions)
      details <- read_csv(file_paths$transaction_items)
      new_details <- data.frame(transaction_item_id = next_id(details, "transaction_item_id"), transaction_id = tx_id, item_id = cart()$item_id, unit_price = cart()$price)
