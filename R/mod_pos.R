...
mod_pos_server <- function(id, State_ID, changed = reactiveVal(0)) {
  moduleServer(id, function(input, output, session) {
    cart <- reactiveVal(data.frame(item_id = integer(), name = character(), original_price = numeric(), stringsAsFactors = FALSE))
    ...
    output$items <- renderDT({
      datatable(available()[, c("name", "price"), drop = FALSE], selection = "single", rownames = FALSE)
    })

    observeEvent(input$items_rows_selected, {
      row <- available()[input$items_rows_selected, , drop = FALSE]
      if (nrow(row)) cart(rbind(cart(), data.frame(item_id = row$item_id, name = row$name, original_price = row$price, stringsAsFactors = FALSE)))
    })

    discounted_cart <- reactive({
      data <- cart()
      discount_rate <- as.numeric(input$discount %||% 0) / 100
      data$price <- if (nrow(data)) round(data$original_price * (1 - discount_rate), 2) else numeric()
    ...
    output$discounted_total <- renderText(paste0("Total after ", input$discount %||% 0, "% discount: ", currency(total())))
